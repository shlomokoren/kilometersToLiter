...
async function findExistingFile(drive) {
  let parentId;
  for (const folder of FOLDER_PATH) {
    const existing = await findChild(drive, folder, parentId, FOLDER_MIME);
    if (!existing) return null;
    parentId = existing.id;
  }
  const file = await findChild(drive, FILE_NAME, parentId);
