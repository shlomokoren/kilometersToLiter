  ...
  if (!schemaReady) {
    schemaReady = getPool()
      .query(
        ...
        ALTER TABLE entries ADD COLUMN IF NOT EXISTS price DOUBLE PRECISION;
...
async function addEntry(email, entry) {
  await ensureSchema();
  await getPool().query(
    `INSERT INTO entries (email, start_km, end_km, liters, price, distance, km_per_l, l_per_100km, mpg_us, entry_date, car_id)
...
async function updateEntry(email, id, entry) {
  ...
  const { rowCount } = await getPool().query(
    `UPDATE entries
     SET start_km = $1, end_km = $2, liters = $3, price = $4, distance = $5,
         km_per_l = $6, l_per_100km = $7, mpg_us = $8, car_id = $9
     WHERE email = $10 AND id = $11`,
    [
      entry.startKm,
      entry.endKm,
      entry.liters,
...
async function getIssueById(id) {
  await ensureSchema();
  const { rows } = await getPool().query('SELECT * FROM issues WHERE id = $1', [id]);
